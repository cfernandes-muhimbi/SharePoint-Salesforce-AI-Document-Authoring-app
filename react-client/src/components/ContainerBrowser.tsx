...
export async function loader({ params }: ILoaderParams): Promise<IContainerLoader> {
    ...
    const rawItemId = params.itemId as string || 'root';
    // In SPE containers the root drive item's ID can equal the driveId.
    // Graph API requires 'root' (not the drive ID) for root-level operations.
    const itemId = (rawItemId === containerId) ? 'root' : rawItemId;
...
export const ContainerBrowser: React.FunctionComponent = () => {
    ...
    const {containerId, itemId: rawItemId = 'root'} = useParams();
    const itemId = (rawItemId === containerId) ? 'root' : rawItemId;
